...
CASES = [
    ...
    # Дефект, который поймал только CI: пустая цепочка каталогов внутри tests/
    # (tests/fixtures/... без файлов) не убиралась, локально каталогов было 45,
    # в CI (где git пустые каталоги не хранит) — 44, и числа NOTICE расходились.
    # Дефект, который поймал только CI: числа tests/ в NOTICE считались по диску,
    # а пустой каталог git не хранит — локально 45, в CI 44. Проверка теперь
    # считает по git ls-files, и расхождение в NOTICE ловится.
    ("M23: число каталогов tests/ в NOTICE разошлось с git",
     [("NOTICE.md", "в 44 каталогах", "в 45 каталогах")],
     [["scripts/validate.py"]]),
