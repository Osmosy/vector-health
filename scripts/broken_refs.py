...
Часть таких ссылок битая — унаследовано от апстримов: файла нет ни в HEAD источника, ни в его истории —
он либо не был закоммичен, либо лежал в служебном каталоге, который убран при
сборке (`tests/`, `evals/` — см. `sync_upstreams.py --prune`). Слепо
...
def write_report(buckets, total_ok: int, origin: dict[str, str]) -> None:
    ...
    lines = [
        ...
        "",
        "Битые ссылки — унаследованное свойство апстримов: файла, который навык упоминает, "
        "нет ни в их HEAD, ни в истории — он либо не был закоммичен, либо лежал в "
        "служебном каталоге, убранном при сборке (`tests/`, `evals/`). Тестовые файлы, "
        "на которые навык ССЫЛАЕТСЯ, при этом сохранены — см. `scripts/service_artifacts.py`. "
        "Правится не ссылка в чужом тексте, а знание о том, где файл есть.",
        "",
