...
# Порядок работы (записано после CI-падения): for-review.md называет КОММИТ, поэтому
# его сборка — ПОСЛЕДНИЙ шаг, после коммита. Если собрать его до коммита, файл
# зафиксирует предыдущий HEAD, и проверка в CI («файл воспроизводится из дерева»)
# упадёт: в коммите лежит ссылка на старый коммит. Локально это не видно — рабочий
# HEAD совпадает с записанным, а в клоне CI — нет.
printf '\nпорядок: собери for-review ПОСЛЕ коммита и закоммить отдельно\n' 
