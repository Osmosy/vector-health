...
- **Битые ссылки не «чистятся».** 492 битых ссылок при 3528 в дереве, разобраны по причинам
  ...
  227 — файла нет ни в источнике, ни у соседа (часть описывает результат работы,
  но отличить это автоматически нельзя); 81 — внешние ресурсы (git-подмодуль
  `omicverse_guide`, каталоги `/opt` и запуска `src/`, `output_dir/`); 75 —
  ссылка ведёт к соседнему навыку, на файл ДРУГОГО навыка (у `guide.md` в апстриме 17 копий и 17 разных
  версий, у `scripts/main.py` — 176 версий на 184 навыка): копировать его нельзя; 73 —
  каталог навыка в источнике есть, но подкаталогов апстрим не выкладывал; 14 — не ссылки,
  а форма пути (`YYYY`, `xxx`); 11 — файл с таким именем в навыке есть, но по другому пути;
  9 — файл в корне репозитория-источника; 2 — демо-датасеты,
  которые не тянем. Пять ссылок закрыты копиями общих файлов апстрима (`scripts/plant_sibling_files.py` кладёт рядом
  файл, который делят ≥3 навыка и потому одинаков у них; пары и sha — в `scripts/sibling-copies.json`).
