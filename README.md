...
- **Битые ссылки не «чистятся».** 497 битых ссылок при 3528 в дереве, разобраны по причинам
  ...
  `src/`, `output_dir/`): такого файла здесь быть не может;
  80 — файл лежит у **соседнего навыка** библиотеки под тем же относительным
  путём (общий файл апстрима, сверено по blob SHA — все совпали);
  73 — **апстрим не публиковал** каталог: навык в источнике есть, но подкаталогов
  (`references/`, `scripts/`, `data/`) он не выкладывал — файла не было и в момент сборки;
  11 — **путь разошёлся**: файл с таким именем в навыке есть, но по другому пути;
  9 — файл в корне репозитория-источника, вне каталога навыка;
  227 — файла нет ни в источнике, ни у соседа; часть описывает результат работы
  (выходные `data/*.vcf.gz`), но отличить это автоматически нельзя, поэтому они не выдаются
  за «создаётся при работе»; 14 — не ссылки, а форма пути (`YYYY`, `xxx`);
  2 — демо-датасеты, которые не тянем. Правка чужого текста не сделала бы навык
  рабочим, поэтому вместо неё — инвентарь с причиной, состоянием и местом, откуда ресурс берётся.
