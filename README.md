...
- **Битые ссылки не «чистятся».** 492 битых ссылок при 3528 в дереве, разобраны по причинам
  ...
  81 — внешние ресурсы (git-подмодуль `omicverse_guide`, каталоги `/opt` и запуска
  `src/`, `output_dir/`); 227 — файла нет ни в источнике, ни у соседа (часть описывает
  результат работы, но отличить это автоматически нельзя, поэтому они не выдаются за «создаётся при работе»);
  73 — каталог навыка в источнике есть, но подкаталогов апстрим не выкладывал;
  72 — ссылка ведёт к соседнему навыку, на файл ДРУГОГО навыка: у `guide.md` в апстриме 17 копий и
  17 разных версий, у `scripts/main.py` — 176 версий на 184 навыка, поэтому копировать его нельзя;
  14 — не ссылки, а форма пути (`YYYY`, `xxx`); 11 — файл
  с таким именем в навыке есть, но по другому пути; 9 — файл в корне репозитория-источника; 3 — ссылку закрывают копии (`scripts/plant_sibling_files.py`
  кладёт рядом файл, который делят ≥3 навыка апстрима и потому одинаков у них); 2 —
  демо-датасеты, которые не тянем. Правка чужого текста не сделала бы навык рабочим, поэтому вместо неё —
  инвентарь с причиной, состоянием и местом, откуда ресурс берётся.
...
| 2026-09-19 | Разбор 394 «унаследованных» ссылок: 11 категорий, 5 копий общих файлов разложены, чужой контент не копируется |
