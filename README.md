...
│   ├── exclusions.py         # служебный ли файл — единственное определение правил
...
│   ├── validate.py            # валидатор репозитория: 30 проверок (спина CI)
...
│   ├── build_for_review.py    # сборка docs/for-review.md: хеши и числа для внешней проверки
│   ├── sync_doc_numbers.py    # числа из stats.json в документы (--check для CI)
│   ├── build_hub_card.py      # карточка Vector Health на сайте экосистемы — из stats.json
│   ├── acceptance.sh          # приёмка одной командой: проверки + офлайн-прогон
...
│   └── offline.md             # как проверять без сети (контейнер/--unshare-net)
...
| 2026-09-20 | Правки по внешнему аудиту, раунд 2: восстановлен затёртый NOTICE (+защита структуры), правила исключения в одном модуле (`exclusions.py`), честный офлайн (`VH_OFFLINE` + изоляция в CI), `trials-verified.json` без чужих аннотаций, числа документов из `stats.json` (`sync_doc_numbers.py`), приёмка одной командой |
