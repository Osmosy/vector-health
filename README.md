...
- **Битые ссылки не «чистятся».** 500 битых ссылок при 3528 в дереве, разобраны по причинам:
  81 — **внешние ресурсы** (git-подмодуль `omicverse_guide` → сторонние тетрадки,
  каталоги `/opt` и запуска `src/`, `output_dir/`): такого файла в репозитории быть не может,
  у 24 ссылок путь совпадает с внешним проектом, 8 переехали, 2 в проекте отсутствуют;
  9 — файл ЕСТЬ в репозитории-источнике, но в его корне (`scripts/`,
  `examples/`, `docs/`): путь писался под раскладку, где каталоги навыков лежат глубже, а у нас
  они плоские; 394 — файлов нет ни у одного источника (у апстримов лежали в
  отрезанных `tests/`/`evals/` или не были закоммичены); 14 — не ссылки,
  а форма пути (`YYYY`, `xxx`); 2 — демо-датасеты, которые не тянем.
  Правка чужого текста не сделала бы навык рабочим, поэтому вместо неё — инвентарь
  с причиной, состоянием и местом, откуда ресурс берётся: `docs/broken-refs.md`.
