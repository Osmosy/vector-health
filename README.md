...
- **Битые ссылки не «чистятся».** 492 битых ссылок при 3528 в дереве, разобраны по причинам
  ...
  14 — не ссылки, а форма пути (`YYYY`, `xxx`); 11 — файл с таким именем в навыке есть, но по
  другому пути; 9 — файл в корне репозитория-источника; 2 — демо-датасеты, которые не тянем.
