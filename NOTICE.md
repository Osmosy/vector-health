...
- **Служебных результатов работы апстримов.** Отчёты прогонов (`eval_report_*`),
  ...
  (8.4 МБ)**, из них по имени 743 и 357 внутри тестовых каталогов.
- **Тестовых каталогов, на которые НИКТО не ссылается.** Из 504 файлов в `tests/`
  осталось **146**: файл каталога
  сохраняется, если его имя или путь встречается в `SKILL.md` или в `references/` —
  так, `LightGBM-analysis` описывает работу на `tests/data/dt_sample1.csv`.
  Пустые каталоги (67) убраны: каталог без файлов — след уборки, а не материал.
